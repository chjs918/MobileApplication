    ...
    LocationCallback mLocCallback = new LocationCallback() {
        ...
        public void onLocationResult(@NonNull LocationResult locationResult) {
            ...
            for(Location loc: locationResult.getLocations()){
                ...
                setTvText(String.format("(%.6f, %.6g)",lat,lng));
                mLastLocation = loc;
                executeGeocoding(mLastLocation);
            }
        }
                ...
                new OnSuccessListener<Location>() {
                    ...
                    public void onSuccess(Location location) {
                        ...
                        if(location != null) {
                            ...
                            setTvText(String.format("최종위치: (%.6f, %.6f", latitude, longtitude));
...
//        Geocoder 객체 생성
        ...
        protected List<Address> doInBackground(Location... locations) {
            List<Address> addresses = null;
               try {
                   //1-> 5로 바꾼이유 : FakeGPS라서 위치 정보가 불분명할수 있어서 있는 주소위치 최대한 받아오기
                   addresses = geocoder.getFromLocation(locations[0].getLatitude(),
                                locations[0].getLongitude(), 5);
            // geocoding 수행
                } catch (IOException e) {
                     e.printStackTrace();
               }
