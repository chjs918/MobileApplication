...
public class MainActivity extends AppCompatActivity {
    ...
    ArrayList<LatLng> addressList = new ArrayList<LatLng>();;
...
//            poiList 의 POI 로 마커 추가 기능 수행
            // new POITask().execute(poiList);
            MarkerOptions poiMarkerOptions = new MarkerOptions();

            int index = 0;
            for (POI p : poiList) {
                if(index > poiList.size()) break;
                new POITask().execute(p);

                if(!addressList.isEmpty()) {
                    //new POITask().execute(p);
                    poiMarkerOptions.position(addressList.get(index))
                            .title(p.getTitle())
                            .snippet(p.getPhone())
                            .icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_AZURE));

                //    Toast.makeText(MainActivity.this, String.format("JISOO"), Toast.LENGTH_SHORT).show();

                    Marker marker = mGoogleMap.addMarker(poiMarkerOptions);
                    marker.setPosition(addressList.get(index));
    ...
    class POITask extends AsyncTask<POI, Void, LatLng> {
        Geocoder geocoder = new Geocoder(MainActivity.this, Locale.getDefault());

        @Override
        protected LatLng doInBackground(POI... poiList) {
            try {
                for (POI p : poiList) {
                    addresses = geocoder.getFromLocationName(p.getAddress(), 1);
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
            ...
            if (addresses == null || addresses.size() == 0) {
                ...
                for (int i = 0; i <= addressList.getMaxAddressLineIndex(); i++) {
                    ...
                    MainActivity.this.addressList.add(latLng);
               }
            }
            return null;
        }
